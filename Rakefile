...
task :clean do
  sh 'rm -rf ~/emmacan.com/output'
end

task :build => [:clean] do
  sh 'magneto --config ~/emmacan.com/config.yaml --source ~/emmacan.com/ --output ~/emmacan.com/output/'
end

task :deploy => [:build] do
  sh 'rsync -avz ~/emmacan.com/output/ dbrunow@208.81.246.43:public/emmacan.com/public/'
end

task :stage => [:clean] do
  sh 'cat config.yaml | sed \'s/^\(base_url:\).*$/\1 http:\/\/stage.emmacan.com/;s/^\(cdn_url:\).*$/\1 ""/;s/^\(google_analytics_account:\).*$/\1 ""/\' >stage_config.yaml && time magneto --config stage_config.yaml && rsync -cavzh --delete --stats --progress output/ stage.emmacan.com:stage.emmacan.com && open http://stage.brunow.org/'
end
